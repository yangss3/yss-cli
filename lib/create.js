...
const addFeatures = require('./utils/addFeatures')
...
module.exports = async function create(projectName) {
  ...
  try {
    ...
    // 选择模板
    const { template, features } = await selectPrompt()
    // 生成模板
    ...
    if (features.length > 0) {
      spinner.start('Configuring features...')
      await addFeatures(features, dist)
      spinner.succeed('Configure features complete.')
    }
    ...
    await execCmd(`git init & git add . & git commit -m 'init'`)
